...
tibble(replicate = 1:5) %>%
  ...
  mutate(n_red = map_int(samp, ~ sum(.$color == "red"))) %>%
  mutate(n_blue = map_int(samp, ~ sum(.$color == "blue"))) %>%
  mutate(red_wins = n_red > n_blue) %>%
  summarize(prob_red_wins = sum(red_wins)/n())
